...
<div class="flex flex-wrap">
    ...
    <x-inputs.group class="w-full">
        <x-inputs.select name="business_type_id" label="Business Type" required>
            ...
            <option disabled {{ empty($selected) ? 'selected' : '' }}>Please select the Business Type
            </option>
    ...
    <x-inputs.group class="w-full">
        <x-inputs.text name="name" label="Name" :value="old('name', $editing ? $kiosk->name : '')" maxlength="255" placeholder="Name" required>
        </x-inputs.text>
    </x-inputs.group>

    <x-inputs.group class="w-full">
        <x-inputs.text name="location" label="Location" :value="old('location', $editing ? $kiosk->location : '')" maxlength="255" placeholder="Location"
            required>
        </x-inputs.text>
    </x-inputs.group>
    ...
    <x-inputs.group class="w-full">
        <x-inputs.text name="comment" label="Comment" :value="old('comment', $editing ? $kiosk->comment : '')" maxlength="255" placeholder="Comment">
        </x-inputs.text>
    </x-inputs.group>
